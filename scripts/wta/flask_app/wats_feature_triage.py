...
def create_wats_ticket(curr_failure, client):
    ...
    labels = ['#E2EAutomation', '#E2EPotentialPB', get_pipeline_triage(curr_failure['pipeline']), '#E2EWeb', feature_assignne_label['label']]
    summary = f"[E2E][Web][{curr_failure['feature_name']}][{curr_failure['test_suite_name']}] - {' '.join(curr_failure['failure_reason'][:100].splitlines())}"
    desc = f'''
    *Description:* {summary}
...
def iterate_matching_failure_for_wats(untriaged_data, triaged_data):
    IMG_DIR = "testImg"
...
iterate_matching_failure_for_wats(untriaged_data, triaged_data)